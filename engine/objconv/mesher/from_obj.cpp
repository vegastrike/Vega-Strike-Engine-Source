...
public:
    float i, j, k;
    TObj_Vector() {}
    TObj_Vector( const GFXVertex &v ) : i( v.x )
        , j( v.y )
        , k( v.z ) {}
    TObj_Vector( float _i, float _j, float _k ) : i( _i )
        , j( _j )
        , k( _k ) {}
...
struct NORMAL
{
    NORMAL( float _i, float _j, float _k ) : i( _i )
        , j( _j )
        , k( _k ) {}
    NORMAL() : i( 1 )
        , j( 0 )
...
{
    FACE() : num( 0 ) {}
    FACE( const IntRef &_r1, const IntRef &_r2 ) : num( 2 )
        , r1( _r1 )
        , r2( _r2 ) {}
    FACE( const IntRef &_r1, const IntRef &_r2, const IntRef &_r3 ) : num( 3 )
        , r1( _r1 )
        , r2( _r2 )
        , r3( _r3 ) {}
    FACE( const IntRef &_r1, const IntRef &_r2, const IntRef &_r3, const IntRef &_r4 ) : num( 4 )
        , r1( _r1 )
        , r2( _r2 )
        , r3( _r3 )
        , r4( _r4 ) {}
                       ...
                       const vector< NORMAL > &normallist )
{
    ...
    if ( e == elements.end() ) {
        ...
        } else {v.i = v.j = v.k = 0; } if (r.t >= 0) {
            v.s = txclist[r.t].s;
            v.t = txclist[r.t].t;
        } else {v.s = v.t = 0; } if (r.v >= 0) {
            v.x = vtxlist[r.v].x;
            v.y = vtxlist[r.v].y;
            v.z = vtxlist[r.v].z;
        } else {v.x = v.y = v.z = 0; } xml.vertices.push_back( v );
