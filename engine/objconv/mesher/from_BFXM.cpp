...
void BFXMToXmeshOrOBJ( FILE *Inputfile, FILE *Outputfile, FILE *OutputObj, FILE *mtl, std::string meshname, char hackfix )
{
    ...
    *inmemfile;
    ...
    if(fread( &bytebuf, sizeof (uchar8bit), 1, Inputfile )!= 1) exit(-1);
    if (bytebuf != 'B') {
    ...
    if(fread( &intbuf, sizeof (uint32bit), 1, Inputfile )!= 1) exit(-1);      //Length of Inputfile
            ...
            for (uint32bit vert = 0; vert < numvertices; vert++) {
                ...
                if (isxmesh) {
                    ...
                    fprintf( OutputObj, "v %f %f %f\nvn %f %f %f\n", scale
                            *(x
                              +transx), scale
                            *(y+transy), scale*(z+transz), i*(flipn ? -1 : 1), j*(flipn ? -1 : 1), k*(flipn ? -1 : 1) );
                }
                if (sharevert) {
                    if (!isxmesh)
