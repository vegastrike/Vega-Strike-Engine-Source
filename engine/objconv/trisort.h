...
public:
    double x, y, z;
    Vector( double xx, double yy, double zz ) : x( xx )
        , y( yy )
        , z( zz ) {}
};
class Index
...
public:
    Vector V;
    int    p, n, t, c;
    Index( Vector xyz, int p, int n, int t, int c ) : V( xyz )
        , p( p )
        , n( n )
        , t( t )
        , c( c ) {}
...
public:
    double a, b, c, d;
    Plane( int a, int b, int c, int d ) : a( a )
        , b( b )
        , c( c )
        , d( d ) {}
...
class Mesh
{
    ...
    std::vector< Vector >c;
    std::vector< Face >  f;
public: Mesh( const char *filename );
    void sort();
