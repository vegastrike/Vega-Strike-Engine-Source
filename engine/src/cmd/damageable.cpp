...
void Damageable::ApplyDamage(const Vector &pnt,
    ...
    if (unit->Destroyed()) {
        ...
        // Eject cargo
        const float cargo_eject_percent = configuration()->eject.eject_cargo_percent;
        const uint32_t max_dump_cargo = configuration()->eject.max_dumped_cargo;
...
//        const float hull_dam_to_eject = configuration()->eject.hull_damage_to_eject;
        const float auto_eject_percent = configuration()->eject.auto_eject_percent;
        const bool player_autoeject = configuration()->eject.player_auto_eject;
