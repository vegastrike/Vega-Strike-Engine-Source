...
{
    v += ( a/parent->Mass )*simulation_atom_var;
    if ( (!max_speed) || fabs( v ) <= max_speed )
        return true;
    float deltaa = parent->Mass*(fabs( v )-max_speed)/simulation_atom_var;       //clamping should take care of it
    a += (v > 0) ? -deltaa : deltaa;
    return false;
}
...
bool MoveToParent::Execute( Unit *parent, const QVector &targetlocation )
{
    ...
    if (terminatingX > switchbacks
        ...
        thrust = (-parent->Mass/simulation_atom_var)*last_velocity;
    } else {
        float div  = 1.0f;
        float vdiv = 1.0f;
        ...
        float t = CalculateDecelTime( heading.k, last_velocity.k, thrust.k, parent->limits.retro/div, parent->Mass );
        if (t < THRESHOLD) {
            thrust.k =
        ...
        t = CalculateBalancedDecelTime( heading.i, last_velocity.i, thrust.i, parent->Mass );
        if (t < THRESHOLD)
            thrust.i = -thrust.i;
        else if (t < simulation_atom_var)
            thrust.i *= ( t-(simulation_atom_var-t) )/simulation_atom_var;
        OptimizeSpeed( parent, last_velocity.i, thrust.i, max_velocity.i/vdiv );
        t = CalculateBalancedDecelTime( heading.j, last_velocity.j, thrust.j, parent->Mass );
        if (t < THRESHOLD)
            thrust.j = -thrust.j;
        else if (t < simulation_atom_var)
...
void ChangeHeading::Execute()
{
    ...
    if ( AICheat && ( (parent->limits.yaw+parent->limits.pitch)*180/( PI*parent->Mass ) > min_for_no_oversteer )
        && !parent->isSubUnit() ) {
        if (xswitch || yswitch) {
            Vector P, Q, R;
