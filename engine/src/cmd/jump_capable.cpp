...
std::string GenerateAutoError(Unit *me, Unit *targ) {
    if (UnitUtil::isAsteroid(targ)) {
        return configuration()->hud.asteroids_near_message;
    }
    if (targ->isPlanet()) {
        return configuration()->hud.planet_near_message;
    }
    if (targ->getRelation(me) < 0) {
        return configuration()->hud.enemy_near_message;
    }
    return configuration()->hud.starship_near_message;
}
...
bool JumpCapable::AutoPilotToErrorMessage(const Unit *target,
    ...
    if (this != target) {
        if ((end - start).MagnitudeSquared()
            ...
            failuremessage = configuration()->hud.already_near_message;
            return false;
        }
