...
bool WeaponInfo::isMissile() const {
    if (configuration()->hud.projectile_means_missile && this->type == WEAPON_TYPE::PROJECTILE) {
        return true;
    }
    if (!configuration()->hud.projectile_means_missile && this->size >= MOUNT_SIZE::LIGHTMISSILE) {
        return true;
    }
    return false;
}
