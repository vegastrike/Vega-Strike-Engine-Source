...
Movable::Movable() : cumulative_transformation_matrix(identity_matrix),
    ...
    cutsqr = configuration()->ftl.warp_stretch_cutoff * configuration()->ftl.warp_stretch_cutoff;
    outcutsqr = configuration()->ftl.warp_stretch_decel_cutoff * configuration()->ftl.warp_stretch_decel_cutoff;
...
void Movable::AddVelocity(float difficulty) {
    ...
    float warprampuptime = playa ? configuration()->ftl.warp_ramp_up_time : configuration()->ftl.computer_warp_ramp_up_time;
    //Warp Turning on/off
    if (graphicOptions.WarpRamping) {
        ...
        if (unit->ftl_drive.Enabled()) {             //Warp Turning on
            ...
            graphicOptions.RampCounter = configuration()->ftl.warp_ramp_down_time;
        }
        ...
        if (oldrampcounter != 0 && graphicOptions.RampCounter != 0) {
            if (unit->ftl_drive.Enabled()) {             //Warp is turning on before it turned off
                graphicOptions.RampCounter *= (1 - oldrampcounter / configuration()->ftl.warp_ramp_down_time);
    ...
    if (unit->ftl_drive.Enabled() || graphicOptions.RampCounter != 0) {
        ...
        if (graphicOptions.RampCounter != 0) {
            ...
            if (!unit->ftl_drive.Enabled() && graphicOptions.RampCounter > configuration()->ftl.warp_ramp_down_time) {
                graphicOptions.RampCounter = (1 - graphicOptions.RampCounter / warprampuptime) * configuration()->ftl.warp_ramp_down_time;
            }
            ...
            rampmult = (unit->ftl_drive.Enabled()) ? 1.0F
                    ...
                    / configuration()->ftl.warp_ramp_down_time) * (graphicOptions.RampCounter / configuration()->ftl.warp_ramp_down_time);
        }
    ...
    graphicOptions.WarpFieldStrength =
            lastWarpField * configuration()->ftl.warp_memory_effect + (1.0 - configuration()->ftl.warp_memory_effect) * graphicOptions.WarpFieldStrength;
...
Vector Movable::ResolveForces(const Transformation &trans, const Matrix &transmat) {
    ...
    if ((newbig && !oldbig) || (oldoutbig && !newoutbig)) {
        ...
        float tmpsec = oldbig ? configuration()->ftl.warp_stretch_decel_cutoff : configuration()->ftl.warp_stretch_cutoff;
...
//    QVector qv = v.Cast();
    ...
    float minimum_multiplier = configuration()->ftl.warp_multiplier_max * graphicOptions.MaxWarpMultiplier;
    Unit *nearest_unit = nullptr;
    minimum_multiplier = unit->CalculateNearestWarpUnit(minimum_multiplier, &nearest_unit, true);
    float minWarp = configuration()->ftl.warp_multiplier_min * graphicOptions.MinWarpMultiplier;
    float maxWarp = configuration()->ftl.warp_multiplier_max * graphicOptions.MaxWarpMultiplier;
...
//    const float warp_max_effective_velocity = vega_config::GetGameConfig().GetFloat("physics.warpMaxEfVel", default_max_warp_effective_velocity);
    const float warp_max_effective_velocity = configuration()->ftl.max_effective_velocity;
...
double Movable::MaxSpeed() const {
    static const double combat_mode_multiplier = configuration()->physics.combat_mode_multiplier;
    const Unit *unit = vega_dynamic_const_cast_ptr<const Unit>(this);
