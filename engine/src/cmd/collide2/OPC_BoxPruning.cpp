...
using namespace Opcode;
	...
	inline_ void FindRunningIndex(ice_udword& index, float* array, ice_udword* sorted, int last, float max)
	{
		int First=index;
		while(First<=last)
		{
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool Opcode::BipartiteBoxPruning(ice_udword nb0, const AABB** array0, ice_udword nb1, const AABB** array1, Pairs& pairs, const Axes& axes)
{
	// Checkings
	if(!nb0 || !array0 || !nb1 || !array1)	return false;

	// Catch axes
	ice_udword Axis0 = axes.mAxis0;
	ice_udword Axis1 = axes.mAxis1;
	ice_udword Axis2 = axes.mAxis2;
	...
	const ice_udword* Sorted0 = RS0->Sort(MinPosList0, nb0).GetRanks();
	const ice_udword* Sorted1 = RS1->Sort(MinPosList1, nb1).GetRanks();

	// 3) Prune the lists
	ice_udword Index0, Index1;

	const ice_udword* const LastSorted0 = &Sorted0[nb0];
	const ice_udword* const LastSorted1 = &Sorted1[nb1];
	const ice_udword* RunningAddress0 = Sorted0;
	const ice_udword* RunningAddress1 = Sorted1;
	...
	while(RunningAddress1<LastSorted1 && Sorted0<LastSorted0)
	{
		...
		const ice_udword* RunningAddress2_1 = RunningAddress1;
	...
	while(RunningAddress0<LastSorted0 && Sorted1<LastSorted1)
	{
		...
		const ice_udword* RunningAddress2_0 = RunningAddress0;
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool Opcode::CompleteBoxPruning(ice_udword nb, const AABB** array, Pairs& pairs, const Axes& axes)
{
	// Checkings
...
//	float* PosList = new float[nb];
	...
	for(ice_udword i=0;i<nb;i++)	PosList[i] = array[i]->GetMin(Axis0);
PosList[nb++] = MAX_FLOAT;

	// 2) Sort the list
	PRUNING_SORTER* RS = GetCompletePruningSorter();
	const ice_udword* Sorted = RS->Sort(PosList, nb).GetRanks();

	// 3) Prune the list
	const ice_udword* const LastSorted = &Sorted[nb];
	const ice_udword* RunningAddress = Sorted;
	ice_udword Index0, Index1;
...
//		while(RunningAddress<LastSorted && PosList[*RunningAddress++]<PosList[Index0]);
		...
		{
			const ice_udword* RunningAddress2 = RunningAddress;
...
//	float* PosList = new float[nb];
	...
	// 1) Build main list using the primary axis
	for(ice_udword i=0;i<nb;i++)	MinList[i] = array[i]->GetMin(Axis0);
	MinList[nb] = MAX_FLOAT;

	// 2) Sort the list
	PRUNING_SORTER* RS = GetCompletePruningSorter();
	ice_udword* Sorted = RS->Sort(MinList, nb+1).GetRanks();

	// 3) Prune the list
//	const ice_udword* const LastSorted = &Sorted[nb];
//	const ice_udword* const LastSorted = &Sorted[nb-1];
	const ice_udword* RunningAddress = Sorted;
	ice_udword Index0, Index1;
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool Opcode::BruteForceBipartiteBoxTest(ice_udword nb0, const AABB** array0, ice_udword nb1, const AABB** array1, Pairs& pairs)
{
	// Checkings
	if(!nb0 || !array0 || !nb1 || !array1)	return false;

	// Brute-force nb0*nb1 overlap tests
	for(ice_udword i=0;i<nb0;i++)
	{
		for(ice_udword j=0;j<nb1;j++)
		{
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool Opcode::BruteForceCompleteBoxTest(ice_udword nb, const AABB** array, Pairs& pairs)
{
	// Checkings
	if(!nb || !array)	return false;
