...
public:
    ...
    // Row-column access
    //! Returns a row.
    inline_    void GetRow(const uint32_t r, HPoint &p) const {
        p.x = m[r][0];
        p.y = m[r][1];
        p.z = m[r][2];
        p.w = m[r][3];
    ...
    //! Returns a row.
    inline_    const HPoint &GetRow(const uint32_t r) const {
        return *(const HPoint *) &m[r][0];
    }
    //! Returns a row.
    inline_    HPoint &GetRow(const uint32_t r) {
        return *(HPoint *) &m[r][0];
    }
    //! Sets a row.
    inline_    void SetRow(const uint32_t r, const HPoint &p) {
        m[r][0] = p.x;
        m[r][1] = p.y;
        m[r][2] = p.z;
        m[r][3] = p.w;
    ...
    //! Returns a column.
    inline_    void GetCol(const uint32_t c, HPoint &p) const {
        p.x = m[0][c];
        p.y = m[1][c];
        p.z = m[2][c];
        p.w = m[3][c];
    ...
    //! Sets a column.
    inline_    void SetCol(const uint32_t c, const HPoint &p) {
        m[0][c] = p.x;
        m[1][c] = p.y;
        m[2][c] = p.z;
        m[3][c] = p.w;
