...
WEAPON_TYPE getWeaponTypeFromString(std::string type) {
    if(type == "Bolt")      return WEAPON_TYPE::BOLT;
    if(type == "Ball")      return WEAPON_TYPE::BALL;
    if(type == "Beam")      return WEAPON_TYPE::BEAM;
    if(type == "Missile")   return WEAPON_TYPE::PROJECTILE;
