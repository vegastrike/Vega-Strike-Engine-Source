...
void GameMenu::createNetworkControls(GroupControl *serverConnGroup, std::vector<unsigned int> *inputqueue) {
    ...
    acctConnGroup->setHidden(!vs_options::instance().use_account_server);
    serverConnGroup->addChild(acctConnGroup);

    GroupControl *hostConnGroup = new GroupControl;
    hostConnGroup->setId("MultiPlayerHostPort");
    hostConnGroup->setHidden(vs_options::instance().use_account_server);
    ...
    serverInputText->setText(vs_options::instance().server_ip);
    ...
    portInputText->setText(vs_options::instance().server_port);
    ...
    acctserverInput->setText(vs_options::instance().account_server_url);
    ...
    if (!vs_options::instance().password.empty()) {
        usernameInput->setText(vs_options::instance().callsign);
    }
    ...
    passwordInput->setText(vs_options::instance().password);
