...
void StarSystem::LoadXML( const char *filename, const Vector &centroid, const float timeofyear )
{
    ...
    cout << "Comparing xmls\n-----------------------\n";
    sys.compare(xml, xml2);
    cout << "-----------------------\n";

    if(this->name == nullptr) this->name = "Dojo";
    cout << "Processed system2 " << this->name << endl;
...
//    if(xml->moons.size() == 2) {
//        if(*xml->moons[0] == *xml2->moons[1]) cout << "eq!\n";
//        if(*xml->moons[1] == *xml2->moons[0]) cout << "eq!\n";
//    }

//     unsigned int i;
//     for (i = 0; i < xml2->moons.size(); ++i) {
//        if (xml2->moons[i]->isUnit() == PLANETPTR) {
//            Unit *un = NULL;
//            for ( Planet::PlanetIterator iter( (Planet*) xml2->moons[i] ); (un = *iter); iter.advance() )
//                AddUnit( un );
//        } else {
//            AddUnit( xml2->moons[i] );
//        }

//        cout << "Adding unit " << xml2->moons[i]->getFullname() << endl;
//    }
