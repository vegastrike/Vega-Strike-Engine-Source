...
int GetNumNearStarsScale() {
    return vs_options::instance().num_near_stars;
}

int GetNumStarsScale() {
    return vs_options::instance().num_far_stars;
}

float GetStarSpreadScale() {
    return vs_options::instance().star_spreading * vs_options::instance().game_speed;
}

float ScaleOrbitDist(bool autogenerated) {
    if (autogenerated) {
        return vs_options::instance().autogen_compactness * vs_options::instance().runtime_compactness;
    }
    return vs_options::instance().runtime_compactness;
}
...
void StarSystem::LoadXML(const string filename, const Vector &centroid, const float timeofyear) {
    ...
    if (vs_options::instance().game_speed_affects_autogen_systems) {
        autogenerated = false;
    }
    ...
    xml->scale *= vs_options::instance().star_system_scale;
    ...
    xml->reflectivity = vs_options::instance().reflectivity;
    xml->unitlevel = 0;
