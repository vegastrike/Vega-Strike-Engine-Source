...
#endif
    if ((vs_options::instance().Music && !music) || (vs_options::instance().Music && music)) {
        ALuint *wavbuf = NULL;
        std::string hashname;
        if (!music) {
...
void AUDAdjustSound(const int sound, const QVector &pos, const Vector &vel) {
    ...
    if (sound >= 0 && sound < (int) sounds.size()) {
        ...
        if (usepositional && sounds[sound].source) {
            ...
            if (!relative) {
                // Set rolloff factrs
                alSourcef(sounds[sound].source, AL_MAX_DISTANCE, scalepos * vs_options::instance().audio_max_distance);
                alSourcef(sounds[sound].source, AL_REFERENCE_DISTANCE, scalepos * vs_options::instance().audio_ref_distance);
