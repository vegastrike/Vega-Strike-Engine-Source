...
//    return 3;
    if (vs_config) {
        return (vs_options::instance().debug_fs);
    }
    return 0;
}
...
FILE* vs_open(const char* filename, const char* mode) {
    ...
    FILE* fp = nullptr;
    bool found = false;
    boost::filesystem::path homedir_path{ homedir };
    boost::filesystem::path datadir_path{ datadir };
    boost::filesystem::path filename_path{ filename };
    boost::filesystem::path fullpath;

    if (!use_volumes && (std::string(mode) == "rb" || std::string(mode) == "r")) {
        try {
            fullpath = boost::filesystem::canonical(filename_path, homedir_path);
            found = true;
        ...
        if (!found) {
            try {
                fullpath = boost::filesystem::canonical(filename_path);
                found = true;
            } catch (boost::filesystem::filesystem_error& e) {
                found = false;
            }
        ...
        if (!found) {
            try {
                fullpath = boost::filesystem::canonical(filename_path, datadir_path);
                found = true;
            } catch (boost::filesystem::filesystem_error& e) {
                found = false;
            }
        ...
        if (found) {
            std::string fullpath_string = fullpath.string();
            const char* fullpath_c_string = fullpath_string.c_str();
            fp = fopen(fullpath_c_string, mode);
            if (!fp) {
                VS_LOG(error, (boost::format("File '%1%' appears to exist, but we weren't able to open it.") % fullpath_string));
            } else if (VSFS_DEBUG() > 2) {
                VS_LOG(debug, (boost::format("SUCCESS opening file '%1%'") % fullpath_string));
            }
        } else {
            VS_LOG(error, (boost::format("File '%1%' NOT FOUND") % filename));
        }
    } else {
        try {
            fullpath = boost::filesystem::canonical(filename_path, homedir_path);
        ...
        if (found) {
            ...
            if (fp) {
                if (VSFS_DEBUG() > 2) {
                    VS_LOG(debug, (boost::format("'%1%' opened for writing SUCCESS") % fullpath_string));
                }
            }
        } else {
            VS_LOG(error, (boost::format("Opening '%1%' FAILED !!!") % filename));
        }
    }
    return fp;
}
...
void LoadConfig(string subdir) {
    ...
    if (!vs_options::instance().datadir.empty()) {
        //We config_file_found a path to data in config file
        VS_LOG(important_info, (boost::format("DATADIR - Found a datadir in config, using : %1%") % vs_options::instance().datadir));
        datadir = vs_options::instance().datadir;
        data_dir_found = true;
    } else {
        if (true == legacy_data_dir_mode) {
...
void InitMods() {
    ...
    if (!vs_options::instance().hqtextures.empty()) {
        ...
        for (boost::filesystem::directory_entry& entry : boost::filesystem::directory_iterator(p)) {
            ...
            if (filename_string == vs_options::instance().hqtextures && is_directory(entry.status())) {
                curpath.clear();
...
//                if (dname == vs_options::instance().hqtextures) {
//                    curpath.clear();
//                    curpath.append(selectcurrentdir);
//                    curpath.append("/");
...
void InitPaths(std::string conf, std::string subdir) {
    ...
    */
    ...
    vs_options::instance().init();

    sharedsectors = vs_options::instance().sectors;
    sharedcockpits = vs_options::instance().cockpits;
    shareduniverse = vs_options::instance().universe_path;
    sharedanims = vs_options::instance().animations;
    sharedvideos = vs_options::instance().movies;
    sharedsprites = vs_options::instance().sprites;
    savedunitpath = vs_options::instance().serialized_xml;
    sharedtextures = vs_options::instance().sharedtextures;
    sharedsounds = vs_options::instance().sharedsounds;
    sharedmeshes = vs_options::instance().sharedmeshes;
    sharedunits = vs_options::instance().sharedunits;
    aidir = vs_options::instance().ai_directory;
    universe_name = vs_options::instance().galaxy;
    ...
    SIMULATION_ATOM = vs_options::instance().simulation_atom;
    simulation_atom_var = SIMULATION_ATOM;
    AUDIO_ATOM = vs_options::instance().audio_atom;
    ...
    volume_format = vs_options::instance().volume_format;
