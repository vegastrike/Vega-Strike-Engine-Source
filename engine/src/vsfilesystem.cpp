...
std::string GetHashName(boost::string_view name) {
    std::string result;
    result = current_path.back() + current_directory.back() + current_subdirectory.back() + name.to_string();
    return result;
}

std::string GetHashName(boost::string_view name, const Vector &scale, int faction) {
    std::string result;
    result = current_path.back() + current_directory.back() + current_subdirectory.back() + name.to_string();
...
std::string GetSharedTextureHashName(boost::string_view name) {
    return std::string("#stex#") + name.to_string();
}

std::string GetSharedSoundHashName(boost::string_view name) {
    return std::string("#ssnd#") + name.to_string();
}

std::string GetSharedMeshHashName(boost::string_view name, const Vector &scale, int faction) {
    return string("#") + XMLSupport::VectorToString(scale) + string("#") + name.to_string() + string("#")
            + XMLSupport::tostring(faction);
}

std::string MakeSharedStarSysPath(boost::string_view s) {
    return MakeSharedStarSysPath(s.to_string());
}

std::string MakeSharedPath(boost::string_view path) {
    return MakeSharedPath(path.to_string());
}

std::string GetCorrectStarSysPath(boost::string_view name, bool &autogenerated) {
