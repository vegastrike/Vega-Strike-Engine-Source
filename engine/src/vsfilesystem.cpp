...
std::string VSFileSystem::GetHashName(boost::string_view name) {
    std::string result;
    result = current_path.back() + current_directory.back() + current_subdirectory.back() + name.to_string();
    return result;
}

std::string VSFileSystem::GetHashName(boost::string_view name, const Vector &scale, int faction) {
    std::string result;
    result = current_path.back() + current_directory.back() + current_subdirectory.back() + name.to_string();
...
std::string VSFileSystem::GetSharedTextureHashName(boost::string_view name) {
    return std::string("#stex#") + name.to_string();
}

std::string VSFileSystem::GetSharedSoundHashName(boost::string_view name) {
    return std::string("#ssnd#") + name.to_string();
}

std::string VSFileSystem::GetSharedMeshHashName(boost::string_view name, const Vector &scale, int faction) {
    return string("#") + XMLSupport::VectorToString(scale) + string("#") + name.to_string() + string("#")
            + XMLSupport::tostring(faction);
}

std::string VSFileSystem::MakeSharedStarSysPath(boost::string_view s) {
    return MakeSharedStarSysPath(s.to_string());
}

std::string VSFileSystem::MakeSharedPath(boost::string_view path) {
    return MakeSharedPath(path.to_string());
}

std::string VSFileSystem::GetCorrectStarSysPath(boost::string_view name, bool &autogenerated) {
    return GetCorrectStarSysPath(name.to_string(), autogenerated);
}
