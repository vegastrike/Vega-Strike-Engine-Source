...
float GameCockpit::LookupUnitStat(int stat, Unit *target) {
    ...
    switch (stat) {
        ...
        case UnitImages<void>::WARPENERGY: {
            const bool warpifnojump = configuration()->graphics_config_.hud.display_warp_energy_if_no_jump_drive;
        ...
        case UnitImages<void>::MAXCOMBATABKPS: {
            const bool use_relative_velocity = configuration()->graphics_config_.hud.display_relative_velocity;
            float value;
            switch (stat) {
                case UnitImages<void>::KPS:
...
GameCockpit::GameCockpit(const char *file, Unit *parent, const std::string &pilot_name)
    ...
    draw_all_boxes = configuration()->graphics_config_.hud.draw_all_target_boxes;
    draw_line_to_target = configuration()->graphics_config_.hud.draw_line_to_target;
    ...
    const float limit_y = configuration()->graphics_config_.fov;
    smooth_fov = limit_y;
        ...
        if ((un = GetParent())) {
            ...
            if ((retauto = retautoA = un->AutoPilotToErrorMessage(un, false, autoMessage))) {
                ...
                if (autoMessage.length() == 0 && !movedatall) {
                    autoMessage = configuration()->graphics_config_.hud.already_near_message;
                    retauto = false;
...
void GameCockpit::Draw() {
    const bool draw_heading_marker = configuration()->graphics_config_.draw_heading_marker;
    const bool draw_star_destination_arrow = configuration()->graphics_config_.hud.draw_star_direction;
    ...
    const bool draw_any_boxes = configuration()->graphics_config_.hud.draw_targeting_boxes;
    const bool draw_boxes_inside_only = configuration()->graphics_config_.hud.draw_targeting_boxes_inside;
    ...
    const bool crosshairs_on_chasecam = configuration()->graphics_config_.hud.crosshairs_on_chase_cam;
    const bool crosshairs_on_padlock = configuration()->graphics_config_.hud.crosshairs_on_padlock;
    if ((view == CP_FRONT)
        ...
        if (Panel.size() > 0 && Panel.front() && screenshotkey == false) {
            const bool drawCrosshairs = configuration()->graphics_config_.hud.draw_rendered_crosshairs;
            if (drawCrosshairs) {
                float x, y, wid, hei;
                Panel.front()->GetPosition(x, y);
    ...
    if ((un = parent.GetUnit())) {
        ...
        if (view == CP_FRONT
            ...
            if (configuration()->graphics_config_.hud.debug_position) {
                TextPlane tp;
        ...
        {
            Unit *parent = NULL;
            if (configuration()->graphics_config_.hud.draw_arrow_to_target && (parent = this->parent.GetUnit())) {
                Radar::Sensor sensor(parent);
                if ((view == CP_PAN
                        && !configuration()->graphics_config_.hud.draw_arrow_on_pan_cam)
                        || (view == CP_PANTARGET
                                && !configuration()->graphics_config_.hud.draw_arrow_on_pan_target) || (view == CP_CHASE && !configuration()
                        ->graphics_config_
                        .hud
    ...
    if ((un = parent.GetUnit())) {
        ...
        if (tgt) {
            ...
            if (view == CP_VIEWTARGET) {
                const float padlock_view_lag = configuration()->graphics_config_.hud.padlock_view_lag;
                const float padlock_view_lag_inv = 1.0F / padlock_view_lag;
                const float padlock_view_lag_fix = configuration()->graphics_config_.hud.padlock_view_lag_fix_zone;
        ...
        if (view == CP_CHASE && !configuration()->graphics_config_.hud.draw_unit_on_chase_cam) {
        } else {
