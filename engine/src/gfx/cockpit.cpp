...
float GameCockpit::LookupUnitStat(int stat, Unit *target) {
    ...
    switch (stat) {
        ...
        case UnitImages<void>::WARPENERGY: {
            const bool warpifnojump = configuration()->hud.display_warp_energy_if_no_jump_drive;
        ...
        case UnitImages<void>::MAXCOMBATABKPS: {
            const bool use_relative_velocity = configuration()->hud.display_relative_velocity;
            float value;
            switch (stat) {
                case UnitImages<void>::KPS:
...
GameCockpit::GameCockpit(const char *file, Unit *parent, const std::string &pilot_name)
    ...
    draw_all_boxes = configuration()->hud.draw_all_target_boxes;
    draw_line_to_target = configuration()->hud.draw_line_to_target;
    draw_line_to_targets_target = configuration()->hud.draw_line_to_targets_target;
    draw_line_to_itts = configuration()->hud.draw_line_to_itts;
    always_itts = configuration()->hud.draw_always_itts;
        ...
        if ((un = GetParent())) {
            ...
            if ((retauto = retautoA = un->AutoPilotToErrorMessage(un, false, autoMessage))) {
                ...
                if (autoMessage.length() == 0 && !movedatall) {
                    autoMessage = configuration()->hud.already_near_message;
                    retauto = false;
...
void GameCockpit::Draw() {
    ...
    const bool draw_star_destination_arrow = configuration()->hud.draw_star_direction;
    ...
    const bool draw_any_boxes = configuration()->hud.draw_targeting_boxes;
    const bool draw_boxes_inside_only = configuration()->hud.draw_targeting_boxes_inside;
    ...
    const bool crosshairs_on_chasecam = configuration()->hud.crosshairs_on_chase_cam;
    const bool crosshairs_on_padlock = configuration()->hud.crosshairs_on_padlock;
    if ((view == CP_FRONT)
        ...
        if (Panel.size() > 0 && Panel.front() && screenshotkey == false) {
            const bool drawCrosshairs = configuration()->hud.draw_rendered_crosshairs;
            if (drawCrosshairs) {
                float x, y, wid, hei;
                Panel.front()->GetPosition(x, y);
    ...
    if ((un = parent.GetUnit())) {
        ...
        if (view == CP_FRONT
            ...
            if (configuration()->hud.debug_position) {
                TextPlane tp;
                std::string str;
        ...
        {
            Unit *parent = NULL;
            if (configuration()->hud.draw_arrow_to_target && (parent = this->parent.GetUnit())) {
                Radar::Sensor sensor(parent);
                if ((view == CP_PAN
                        && !configuration()->hud.draw_arrow_on_pan_cam)
                        || (view == CP_PANTARGET
                                && !configuration()->hud.draw_arrow_on_pan_target) || 
                                (view == CP_CHASE && !configuration()->hud.draw_arrow_on_chase_cam)) {
                } else {
    ...
    if ((un = parent.GetUnit())) {
        ...
        if (tgt) {
            ...
            if (view == CP_VIEWTARGET) {
                const float padlock_view_lag = configuration()->hud.padlock_view_lag;
                const float padlock_view_lag_inv = 1.0F / padlock_view_lag;
                const float padlock_view_lag_fix = configuration()->hud.padlock_view_lag_fix_zone;
        ...
        if (view == CP_CHASE && !configuration()->hud.draw_unit_on_chase_cam) {
        } else {
