...
VertexBuilder<> GetAnimatedLockingIcon(const QVector &location, const Vector& cam_p,
    ...
    const double r_1_size = std::max(r_size * vega_config::config->graphics.hud.rotating_bracket_size, vega_config::config->graphics.hud.min_lock_box_size);
...
void SetThickness(ShapeType type) {
    switch(type) {
        case ShapeType::Box:
            GFXLineWidth(vega_config::config->graphics.hud.box_line_thickness);
            break;
        case ShapeType::Diamond:
            GFXLineWidth(vega_config::config->graphics.hud.diamond_line_thickness);
            break;
        case ShapeType::Cross:
            GFXLineWidth(vega_config::config->graphics.hud.nav_cross_line_thickness);
            break;
        default:
            GFXLineWidth(1.0F);
            break;
