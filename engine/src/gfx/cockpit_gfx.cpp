                              ...
                              bool Diamond  )
{
    ...
    const float rat = vega_config::config->graphics.hud.min_target_box_size;
...
{
    if (!vega_config::config->graphics.hud.draw_tactical_target)
        return;
                   ...
                   bool always_itts, float locking_percent, bool draw_line_to_itts, bool steady_itts)
{
    ...
    GFXBlendMode( SRCALPHA, INVSRCALPHA );
    GFXDisable( LIGHTING );
    if (vega_config::config->graphics.hud.draw_nav_symbol) {
    ...
    if ( (always_itts || player->GetComputerData().itts) && !nav_symbol ) {
        ...
        const bool itts_for_locks = vega_config::config->graphics.hud.itts_for_lockable;
        const bool itts_for_beams = vega_config::config->graphics.hud.itts_for_beams;
        const bool line_to_itts_alpha = vega_config::config->graphics.hud.itts_line_to_mark_alpha;
        QVector p = CamP.Cast();
        QVector q = CamQ.Cast();
        ...
        GFXColor mntcolor;
        if (vega_config::config->graphics.hud.itts_use_average_gun_speed) {
