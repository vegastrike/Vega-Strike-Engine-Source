                              ...
                              bool Diamond  )
{
    ...
    if (lock_percent < 0.99F) {
        ...
        glLineWidth(configuration()->graphics_config_.hud.diamond_line_thickness);

        if (configuration()->graphics_config_.hud.lock_center_crosshair) {
...
{
    if (!configuration()->graphics_config_.hud.draw_tactical_target)
        return;
    ...
    if (target) {
        ...
        const float fudge = configuration()->graphics_config_.hud.tac_target_length;
        const float foci = configuration()->graphics_config_.hud.tac_target_foci;
        glLineWidth( (int) configuration()->graphics_config_.hud.tac_target_thickness);         //temp
                   ...
                   bool always_itts, float locking_percent, bool draw_line_to_itts, bool steady_itts)
{
    ...
    GFXBlendMode( SRCALPHA, INVSRCALPHA );
    GFXDisable( LIGHTING );
    if (configuration()->graphics_config_.hud.draw_nav_symbol) {
    ...
    if ( (always_itts || player->GetComputerData().itts) && !nav_symbol ) {
        ...
        const bool itts_for_locks = configuration()->graphics_config_.hud.itts_for_lockable;
        const bool itts_for_beams = configuration()->graphics_config_.hud.itts_for_beams;
        const bool line_to_itts_alpha = configuration()->graphics_config_.hud.itts_line_to_mark_alpha;
        QVector p = CamP.Cast();
        QVector q = CamQ.Cast();
        ...
        GFXColor mntcolor;
        if (configuration()->graphics_config_.hud.itts_use_average_gun_speed) {
