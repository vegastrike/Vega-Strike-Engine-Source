...
public:
    ...
    bool acquire(const Unit *target, float distance) {
        ...
        const bool draw_significant_blips = configuration()->hud.draw_significant_blips;
        const bool untarget_out_cone = configuration()->hud.untarget_beyond_cone;
        const float min_radar_blip_size = configuration()->hud.min_radar_blip_size;
...
const Sensor::TrackCollection &Sensor::FindTracksInRange() const {
    ...
    const float kMaxUnitRadius = configuration()->hud.radar_search_extra_radius;
    const bool kDrawGravitationalObjects = configuration()->hud.draw_gravitational_objects;
