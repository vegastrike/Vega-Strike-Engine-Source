...
void Configuration::OverrideDefaultsWithUserConfiguration() {
    ...
    collision_hacks.front_collision_hack_angle = std::cos(M_PI * GameConfig::GetVariable("physics", "front_collision_avoidance_hack_angle", 40) / 180.0f); // uses default value - must be changed in tandem with constructor!
    collision_hacks.front_collision_hack_distance = GameConfig::GetVariable("physics", "front_collision_avoidance_hack_distance", collision_hacks.front_collision_hack_distance);
    collision_hacks.cargo_deals_collide_damage = GameConfig::GetVariable("physics", "cargo_deals_collide_damage", collision_hacks.cargo_deals_collide_damage);
    ...
    // fuel substruct
    fuel.afterburner_fuel_usage = GameConfig::GetVariable("physics", "AfterburnerFuelUsage", fuel.afterburner_fuel_usage);
    fuel.fmec_exit_velocity_inverse = GameConfig::GetVariable("physics", "FMEC_exit_vel", fuel.fmec_exit_velocity_inverse);
    fuel.fuel_efficiency = GameConfig::GetVariable("physics", "LithiumRelativeEfficiency_Lithium", fuel.fuel_efficiency);
    ...
    physics.collision_scale_factor = GameConfig::GetVariable("physics", "collision_damage_scale", physics.collision_scale_factor);
    physics.inelastic_scale = GameConfig::GetVariable("physics", "inelastic_scale", physics.inelastic_scale);
    physics.kilojoules_per_damage = GameConfig::GetVariable("physics", "kilojoules_per_unit_damage", physics.kilojoules_per_damage);
    physics.max_force_multiplier = GameConfig::GetVariable("physics", "maxCollisionForceMultiplier", physics.max_force_multiplier);
    physics.max_shield_lowers_capacitance = GameConfig::GetVariable("physics", "max_shield_lowers_capacitance", physics.max_shield_lowers_capacitance);
    physics.max_torque_multiplier = GameConfig::GetVariable("physics", "maxCollisionTorqueMultiplier", physics.max_torque_multiplier);
    physics.minimum_mass = GameConfig::GetVariable("physics", "minimum_physics_object_mass", physics.minimum_mass);
    physics.minimum_time = GameConfig::GetVariable("physics", "minimum_time_between_recorded_player_collisions", physics.minimum_time);
...
vegastrike_configuration::CollisionHacks::CollisionHacks():
    ...
    front_collision_hack_angle(std::cos( M_PI *  40.0/180.0)), /* Note: Does not follow pattern of directly setting itself to user specified config value, must change default value in override as well if changing it here*/
