...
namespace vega_config {
    ...
    // Actions: command -> per-device binding arrays (replaces the old
    // flat 'controls' struct; the engine fills the same in-memory binding
    // tables from these). See config.json "actions" section.
    struct ActionBinding {
        std::string key;        // keyboard: key name (e.g. "tab", "a")
        std::string modifier;   // "none" | "alt" | "ctrl" (shift is encoded in key)
        int button = -1;        // mouse/joystick: button index
        int joystick = -1;      // joystick: device index (mouse uses -1 + mouse flag)
        bool is_mouse = false;  // mouse button binding (mouse-as-joystick)
        int hatswitch = -1;     // hat: hat index (digital hatswitch)
        std::string direction;  // hat: VS_HAT_* name ("up","down",...)
    };
    ...
    // Axes: role (x/y/z/throttle) -> source device + physical axis + inverse.
    struct AxisRole {
