...
void GFXInitTextureManager() {
    for (size_t handle = 0; handle < textures.size(); ++handle) {
        ...
        textures.at(handle).width = textures.at(handle).height = textures.at(handle).iwidth = textures.at(handle).iheight = 0;
        textures.at(handle).texturestage = 0;
        textures.at(handle).name = 0;
        textures.at(handle).alive = 0;
        textures.at(handle).textureformat = DUMMY;
        textures.at(handle).targets = 0;
        textures.at(handle).mipmapped = NEAREST;
    }
