 ...
 * Current development team: Roy Falk, Benjamen R. Meyer,
 * Stephen G. Tuggy, Evert Vorster
 *
 *
...
void WriteLight(unsigned int i) {
    ...
    ambient *= vs_options::instance().AmbientLightFactor;
    Tab();
    f.Fprintf("<Light>\n");
    xmllevel++;
    Tab();
...
void readColorGrads(vector<string> &entity, const char *file) {
    ...
    while (!f.Eof()) {
        ...
        if (sscanf(input_buffer, "%f %f %f %f %f %s", &g.minrad, &g.r, &g.g, &g.b, &g.variance, output_buffer) == 6) {
            g.minrad *= vs_options::instance().StarRadiusScale;
            colorGradiant.push_back(g);
...
GFXColor getStarColorFromRadius(float radius) {
    ...
    Color tmp = StarColor(radius * vs_options::instance().StarRadiusScale, myint);
...
void MakePlanet(float radius,
    ...
    if (atmosphere == "false") {
        ...
        atmosphere = vs_options::instance().DefaultAtmosphereTexture;
    }
    string cname;
    ...
    if ((entitytype == PLANET && temprandom < vs_options::instance().AtmosphereProbability) && (!atmosphere.empty())) {
...
//FIRME: need scaling of radius based on planet type.
    ...
    if (entitytype == PLANET) {
        ...
        if (ringrand < vs_options::instance().RingProbability) {
            string ringname = getRandName(rings);
            double inner_rad = (vs_options::instance().InnerRingRadius * (1.0 + grand() * 0.5)) * radius;
            double outer_rad = inner_rad + (static_cast<double>(vs_options::instance().OuterRingRadius) * grand()) * radius;
            int wrapx = 1;
            int wrapy = 1;
            if (ringname.empty()) {
                ringname = vs_options::instance().DefaultRingTexture;
            }
            ...
            if (ringrand < (1 - vs_options::instance().DoubleRingProbability)) {
                Tab();
                f.Fprintf(
            ...
            if (ringrand < vs_options::instance().DoubleRingProbability
                    || ringrand >= (vs_options::instance().RingProbability - vs_options::instance().DoubleRingProbability)) {
                double movable = grand();
                inner_rad = outer_rad
                        * (1 + .1 * (vs_options::instance().SecondRingDifference + vs_options::instance().SecondRingDifference * movable));
                outer_rad = inner_rad * (vs_options::instance().OuterRingRadius * movable);
                Tab();
                f.Fprintf(
                        "<Ring file=\"%s\" ri=\"%f\" rj=\"%f\" rk=\"%f\" si=\"%f\" sj=\"%f\" sk=\"%f\" innerradius=\"%f\" outerradius=\"%f\" wrapx=\"%d\" wrapy=\"%d\" />\n",
                        ringname.c_str(),
    ...
    moonlevel++;
    MakeMoons(
            entitytype != MOON ? vs_options::instance().MoonRelativeToPlanet * radius : vs_options::instance().MoonRelativeToMoon * radius,
            entitytype);
...
void beginStar() {
    ...
    MakeMoons(vs_options::instance().RockyRelativeToPrimary * radius, STAR);
...
void generateStarSystem(SystemInfo &si) {
    ...
    si.sunradius *= vs_options::instance().StarRadiusScale;
    systemname = si.name;

    compactness = si.compactness * vs_options::instance().CompactnessScale;
    jumpcompactness = si.compactness * vs_options::instance().JumpCompactnessScale;
    if (si.seed) {
        seedrand(si.seed);
    } else {
    ...
    int nat = pushTowardsMean(vs_options::instance().MeanNaturalPhenomena, si.numun1);
    numnaturalphenomena = nat > si.numun1 ? si.numun1 : nat;
    numstarbases = pushTowardsMean(vs_options::instance().MeanStarBases, si.numun2);
