...
[Currently supported versions are 1.53, 1.63, 1.64 and system]))
case "$with_boost" in
   "") with_boost=1.64 ;;
   "1.53" | "1.63" | "1.64"| "system") ;;
...
if (test "x${with_boost}" != "x1.64"); then
...
AM_CONDITIONAL([BOOST1_53], [test x$with_boost = x1.53])
AM_CONDITIONAL([BOOST1_63], [test x$with_boost = x1.63])
AM_CONDITIONAL([BOOST1_64], [test x$with_boost = x1.64])
