...
(2013-11-29). It is essentially what the 0.5 release should have been: the pre-CSV-split
2013 mechanics, cleaned up and made to build and run on a modern system with **no new
features and no mechanics changes** — only the library and build-system porting needed to
compile and run.

It exists as a clean, known-good baseline of the game mechanics — to demonstrate that the
original game ran well, and as the reference source for the long-term Godot remake.

The two repos (in lockstep at the fork point):
...
```sh
git clone git@github.com:evertvorster/VS-05-lts-Engine.git
git clone git@github.com:evertvorster/VS-05-lts-Assets.git
```

## Dependencies

**Engine build tools:** CMake, a C++ compiler, git, system Boost (headers).

**Engine runtime libraries:** Boost, Python 3, freeglut, SDL3, libvorbis, OpenAL,
libpng, libjpeg-turbo, zlib, expat, GLU.

**Assets:** pure game data — no build; it only needs the engine.

The exact package names differ by distribution (e.g. on Arch: `cmake`, `boost`,
`python`, `freeglut`, `sdl3`, `libvorbis`, `openal`, `libpng`, `libjpeg-turbo`,
`zlib`, `expat`, `glu`).

## Compiling

The classic workflow: **clone the repo, build it in place, and run from there.** Build the
engine first (the assets are data-only). From a fresh checkout:
...
The assets need no build step — run the engine with `-D` pointing at the assets `data`
directory.
...
> a self-contained location so it does not clash with a main Vega Strike install, and adds
> desktop entries for the game and the settings app.

---
The section below is the **original 2002-era README** (gameplay, controls, mission/AI
editing), kept intact for reference.
---
