...
namespace XMLDOM {
    ...
    public:
        ...
        const XMLElement* getParent() const { return mParent; }

        /** Returns the element containing this element, if any - NULL otherwise */
        XMLElement* getParent() { return mParent; }

        /** Returns the element containing this element, if any - NULL otherwise */
        const XMLDocument* getDocument() const { return mDocument; }

        /** Returns the element containing this element, if any - NULL otherwise */
        XMLDocument* getDocument() { return mDocument; }
        ...
        */
        ...
        unsigned int numChildren() const { return mChildren.size(); }


        /** Returns a read-only pointer to the specified children, or NULL if idx is out of range */
        const XMLElement* getChild(unsigned int idx) const { return (idx<mChildren.size())?mChildren[idx]:0; }

        /** Returns a pointer to the specified children, or NULL if idx is out of range */
        XMLElement* getChild(unsigned int idx) { return (idx<mChildren.size())?mChildren[idx]:0; }
        ...
        */
        ...
        /** @copydoc removeChild */
        void removeChildById(const std::string &id) { removeChild(getChildById(id)); }

        /** @copydoc removeChild */
        void removeChildByName(const std::string &name) { removeChild(getChildByName(name)); }
        ...
        const_attribute_iterator getAttribute(const std::string &name) const { return mAttributes.find(name); }

        /** Get an iterator pointing to the specified attribute */
        attribute_iterator getAttribute(const std::string &name) { return mAttributes.find(name); }
        ...
        const std::string& getAttributeValue(const std::string &name, const std::string &def) const 
            {   const_attribute_iterator it = getAttribute(name); return (it==attributesEnd())?def:it->second;  }


        /** Get an iterator pointing to the first attribute */
        attribute_iterator attributesBegin() { return mAttributes.begin(); }
        /** Get an iterator pointing to the first attribute */
        const_attribute_iterator attributesBegin() const { return mAttributes.begin(); }
        /** Get an iterator pointing to the last attribute */
        attribute_iterator attributesEnd() { return mAttributes.end(); }
        /** Get an iterator pointing to the last attribute */
        const_attribute_iterator attributesEnd() const { return mAttributes.end(); }

        /** Get an iterator pointing to the first child */
        child_iterator childrenBegin() { return mChildren.begin(); }
        /** Get an iterator pointing to the first child */
        const_child_iterator childrenBegin() const { return mChildren.begin(); }
        /** Get an iterator pointing to the last child */
        child_iterator childrenEnd() { return mChildren.end(); }
        /** Get an iterator pointing to the last child */
        const_child_iterator childrenEnd() const { return mChildren.end(); }
    ...
    public:
        ...
        */
        virtual std::pair<bool,std::string> execute(XMLSerializer *serializer, XMLDocument *doc, XMLElement *current, const std::string& data) = 0;
    };
