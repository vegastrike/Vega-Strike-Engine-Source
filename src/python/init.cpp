...
{
    std::string country;
public: hello( const std::string &country )
    {
    ...
    virtual ~hello()
    {
        VSFileSystem::vs_fprintf( stderr, "NO HELLO %d", this );
    }                                                                            //Good practice
};
...
struct hello_callback : hello
{
    ...
    {
        VSFileSystem::vs_fprintf( stderr, "NO CALLBAC %d", this );
    }
...
protected:
    int val;
public: MyBaseClass( int set )
    {
        val = set;
    }
    virtual void foo( int set )
    {
        val = 0;
    }
    virtual int get()
...
{
private:
public: MyDerivedClass( int set ) : MyBaseClass( set )
    {
        val = 4364;
    }
    virtual void foo( int set )
...
public:
    struct myattr
    {
        std::string name;
        std::string value;
        myattr( std::string nam, std::string val )
        {
            name  = nam;
            value = val;
        }
...
{
public: MyFA() : CommunicatingAI( 0, 0 ) {}
