...
//cerr << *sel << endl;
    ...
    if (k == DOWN) {
        if (delx || dely) {
            ...
            CurDFA->SelectBox.SetPosition( GetX( CurDFA->prevx )+.5*( GetX( x )-GetX( CurDFA->prevx ) ), GetY(
                                              CurDFA->prevy )+.5*( GetY( y )-GetY( CurDFA->prevy ) ) );

            CurDFA->Selecting = true;
            if (mod&ACTIVE_SHIFT) {
                //do clickb0x0rz on both CurDFA->selection && tmpcol FIXME
                /*CurDFA->appendCollection(tmpcol)*/} else {
...
void InputDFA::NoneSelect( KBSTATE k, int x, int y, int delx, int dely, int mod )
{
    ...
    if (k == PRESS) {
        ...
        if (sel != NULL) {
            UnitCollection *tmpcollection = new UnitCollection();
            tmpcollection->append( sel );
            VSFileSystem::vs_fprintf( stderr, "None::replacing Single Unit" );
...
InputDFA::InputDFA( StarSystem *par ) : MouseArrow( "mouse.spr" )
    , SelectBox( "selectbox.spr" )
{
    parentSystem = par;
...
void InputDFA::ContextAcquire()
{
    ...
    {
    case NONE:
        BindKey( 0, NoneSelect );
        break;
    case UNITS_SELECTED:
        BindKey( 0, ClickSelect );
        break;
    case LOCATION_SELECT:
        BindKey( 0, LocSelect );
        break;
