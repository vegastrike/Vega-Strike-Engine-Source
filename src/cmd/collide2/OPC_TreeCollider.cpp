...
const char* AABBTreeCollider::ValidateSettings()
{
    if( TemporalCoherenceEnabled()
       && !FirstContactEnabled() ) return "Temporal coherence only works with " "First contact" " mode!";
    return null;
}
...
void AABBTreeCollider::_Collide( const AABBNoLeafNode *a, const AABBNoLeafNode *b )
{
    ...
    else
    {
        ...
        else{ _Collide( a->GetPos(), b->GetPos() ); } if( ContactFound() ) return;
        if( BHasNegLeaf )
        {
    ...
    else
    {
        ...
        else{ _Collide( a->GetNeg(), b->GetPos() ); } if( ContactFound() ) return;
        if( BHasNegLeaf )
        {
            //### That leaf has possibly already been fetched
            FETCH_LEAF( b->GetNegPrimitive(), mIMesh1, mR1to0, mT1to0 );
