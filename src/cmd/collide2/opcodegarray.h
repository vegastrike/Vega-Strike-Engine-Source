...
#define CS_TYPEDEF_GROWING_ARRAY_EXT(Name, Type, ExtraConstructor, Extra) \
  ...
  public:								\
    ...
    {									\
      ...
      if ((limit = iLimit)!=0)						\
        { \
        	int oldlength = length < limit ? length : limit;	\
        	ga_type *newroot = new ga_type[limit]; \
        	if (root) \
        	{ \
        		std::copy (root, root + oldlength, newroot);	\
        		delete[] root; \
        	} \
        	root = newroot; \
        } \
	...
	{ \
	  int newlimit = ((iLength + (iGrowStep - 1)) / iGrowStep) * iGrowStep; \
    ...
    { CS_ASSERT (n >= 0 && n < limit);					\
      std::copy (root + n + 1, root + limit, root + n); \
      SetLength (length-1); }						\
    ga_type *GetArray ()						\
    { return root; }							\
    ...
    {									\
      ...
      SetLength (length + 1, iGrowStep);				\
      std::copy_backward (root + pos, root + length - 1, root + length); \
      root [pos] = val;			\
    }									\
    Extra								\
  } \
