...
GameUnit< UnitType >::GameUnit( int ) : sparkle_accum( 0 )
    , phalos()
{
    this->Unit::Init();
}
 ...
 *  {
 *  this->Unit::Init();
 *  //  unsigned int numg= 1+MAXVDUS+UnitImages<void>::NUMGAUGES;
...
//Mesh * gun = WeaponMeshCache::getCachedMutable (mounts[i]->type->weapon_name);
    ...
    if ( phalos->ShouldDraw( mat, velocity, accel, maxaccel, cmas ) && !( this->docked&(UnitType::DOCKED|UnitType::DOCKED_INSIDE) ) )
