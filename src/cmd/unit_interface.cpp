    ...
    , title( "Buy Cargo" )
{
    ...
    const char MyButtonModes[][128] = {
        "Merchant's Inventory", "My Inventory",          "Mission BBS",   "My Ship Stats", "GNN News", "Ship Dealer",
        "Upgrade Ship",
        "Unimplemented",        "Downgrade Ship",        "Save/Load"
    };
    float beginx = -.4;
    float lastx  = beginx;
    float sizeb;
...
void UpgradingInfo::SetupCargoList()
{
    ...
    if (submode == NORMAL) {
        if (mode == SAVEMODE) {
        ...
        } else if (mode == NEWSMODE) {
            static bool news_from_cargolist =
            ...
            if ( !curcategory.empty() ) {
                ...
                for (unsigned int i = 0; i < CurrentList->size(); i++) {
                    if ( match( curcategory.begin(), curcategory.end(), (*CurrentList)[i].cargo.GetCategory().begin(),
                                (*CurrentList)[i].cargo.GetCategory().end(), true ) ) {
                        ...
                        if ( un && ( cpt = _Universe->isPlayerStarship( un ) ) ) {
                            ...
                            bas = base.GetUnit();
                            if (mode == BUYMODE) {
                                if ( ( (*CurrentList)[i].cargo.price > cpt->credits )
                                    || ( !( un->CanAddCargo( (*CurrentList)[i].cargo ) ) ) )
                                    (*CurrentList)[i].color = nomoney;
                            } else if (mode == SHIPDEALERMODE && bas) {
                                if ( ( ( (*CurrentList)[i].cargo.price-( usedPrice( bas->PriceCargo( un->name ) ) ) )
                                      > cpt->credits ) )
                                    (*CurrentList)[i].color = nomoney;
                        ...
                        CargoList->AddTextItem( ( tostring( (int) i )+string(
                                                     " " )+(*CurrentList)[i].cargo.GetContent() ).c_str(),
                                               printstr.c_str(), NULL, (*CurrentList)[i].color );
                        addedsomething = true;
                    }
                }
            }
            ...
            for (unsigned int i = 0; i < CurrentList->size(); i++) {
                ...
                if (match( curcategory.begin(), curcategory.end(), curlist.begin(), curlist.end(), false )
                    ...
                    && lev != curcat) {
                    GFXColor col = (mode != SHIPDEALERMODE || lev != "My_Fleet") ? GFXColor( 0, 1, 1, 1 ) : GFXColor( 0,
                                                                                                                      .5,
                                                                                                                      1,
                                                                                                                      1 );
            ...
            case MOUNT_MODE:
                for (; i < un->GetNumMounts(); i++) {
                    if (un->mounts[i].status == Mount::ACTIVE || un->mounts[i].status == Mount::INACTIVE) {
                        CargoList->AddTextItem( (tostring(
                                                     i )+un->mounts[i].type->weapon_name).c_str(),
                                               un->mounts[i].type->weapon_name.c_str() );
                    } else {
                        CargoList->AddTextItem( (tostring( i )+" [Empty]").c_str(),
                                               ( std::string( "[" )+lookupMountSize( un->mounts[i].size )+std::string(
                                                    "]" ) ).c_str() );
                    }
                }
                break;
...
void UpgradeCompInterface( Unit *un, Unit *base, vector< UpgradingInfo::BaseMode >modes )
{
    if (upgr)
        if (upgr->buyer.GetUnit() == un)
            return;
    //too rich for my blood...don't let 2 people buy cargo for 1
...
void CargoToMission( const char *item, TextArea *ta, Unit *buyer )
{
    ...
    static bool miss_from_cargolist =
...
string BasicRepair( Unit *parent, string title )
{
    if (parent) {
        ...
        } else {newtitle = "Not Enough Credits to Repair"; } return newtitle;
    }
    return title;
}
...
void BuyShip( Unit *base, Unit *un, std::string input_buffer, bool my_fleet, UnitContainer *buyer, UpgradingInfo *baseinfo )
{
    ...
    if (part) {
        ...
        if (part->price <= usedprice+_Universe->AccessCockpit()->credits) {
            ...
            Unit  *NewPart = UnitFactory::createUnit(
                input_buffer.c_str(), false, base->faction, newmodifications, fg, fgsnumber );
            ...
            if ( NewPart->name != string( "LOAD_FAILED" ) ) {
                if (NewPart->nummesh() > 0) {
                    ...
                    _Universe->AccessCockpit()->SetParent( NewPart, input_buffer.c_str(),
                                                           _Universe->AccessCockpit()->GetUnitModifications().c_str(),
                                                           un->curr_physical_state.position );                                                                                                           //absolutely NO NO NO modifications...you got this baby clean off the slate
...
void UpgradingInfo::CommitItem( const char *inp_buf, int button, int state )
{
    ...
    if ( state == 0 && ( un = buyer.GetUnit() ) && ( base = this->base.GetUnit() ) ) {
        ...
        case SHIPDEALERMODE:
            {
                BuyShip( base, un, input_buffer, std::find( curcategory.begin(), curcategory.end(), string(
                ...
                if (!temprate) {
                    temprate =
                        UnitConstCache::setCachedConst( StringIntKey( templnam,
                                                                      un->faction ),
                                                       UnitFactory::createUnit( templnam.c_str(), true, un->faction ) );
                }
                free( unitdir );
    ...
    if ( ( un = buyer.GetUnit() ) ) {
        ...
        if (!canupgrade) {
            title = (mode == DOWNGRADEMODE) ? string( "You do not have exactly what you wish to sell. Continue?" ) : string(
                "The upgrade cannot fit the frame of your starship. Continue?" );
...
void UpgradingInfo::SelectLastSelected()
{
    if (lastselected.last) {
        int ours = CargoList->DoMouse( lastselected.type,
                                       lastselected.x,
                                       lastselected.y,
                                       lastselected.button,
                                       lastselected.state );
        if (ours) {
            ProcessMouse( lastselected.type, float_to_int( lastselected.x ), float_to_int(
                              lastselected.y ), lastselected.button, lastselected.state );
...
void UpgradingInfo::ProcessMouse( int type, int x, int y, int button, int state )
{
    ...
    if (ours == 0) {
        ...
        if (ours == 1 && type == 1) {
            ...
            } else if (submode == STOP_MODE && mode == BRIEFINGMODE) {
                StopBriefing();
            }
        }
    }
...
vector< CargoColor >& UpgradingInfo::MakeActiveMissionCargo()
{
    ...
    for (unsigned int i = 0; i < active_missions.size(); i++) {
        ...
        c.cargo.volume   = 1;
        c.cargo.price    = 0;
        c.cargo.content  = XMLSupport::tostring( (int) i )+" "
                           +active_missions[i]->getVariable( "mission_name", string( "Mission" ) );
...
vector< CargoColor >& UpgradingInfo::FilterCargo( Unit *un, const string filterthis, bool inv, bool removezero )
{
    ...
    for (unsigned int i = 0; i < un->numCargo(); i++) {
        ...
        if ( ( 0 == memcmp( un->GetCargo( i ).GetCategory().c_str(), filterthis.c_str(), len ) ) == inv ) {
            //only compares up to category...so we could have starship_blue
...
vector< CargoColor >& UpgradingInfo::GetCargoFor( Unit *un )
{
    //un !=NULL
    switch (mode)
    {
