...
ObjSerial	getUniqueSerial()
{
	...
	size_t maxserial=(((size_t)1)<<(sizeof(ObjSerial)*8));
	if (usedSerials.size()>=maxserial/3) {
		//serials advance in steps of 3, so only maxserial/3 exist per offset class:
		//once they are all used the loop below would spin forever--better a bogus serial than a hang
