...
static void nav_keyboard_cb( unsigned int  ch,unsigned int mod, bool release, int x, int y ) {
  if (!release)
    nav_keyboard_queue.push_back (((WSK_MOD_LSHIFT==(mod&WSK_MOD_LSHIFT))||(WSK_MOD_RSHIFT==(mod&WSK_MOD_RSHIFT)))?shiftup(ch):ch);
}
...
bool NavComputer::actionChain(const EventCommandId& command, Control* control) {
  ...
  ValuedPickerCell<ChainPathNode::PartType>* typeCell = static_cast<ValuedPickerCell<ChainPathNode::PartType>*>(typePicker->selectedCell());
