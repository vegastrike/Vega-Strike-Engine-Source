...
class BSPDiskNodeArray
{
    ...
    size_t bounds;
    size_t cur;
public: BSPDiskNodeArray( BSPDiskNode *array, size_t bounds )
    {
        this->array        = array;
        this->bounds       = bounds;
        this->cur          = 0;
    ...
    BSPDiskNode* operator*()
    {
        ...
        fprintf(
            stderr,
            "Serious BSP Building Fault:: Array out of bounds! and accessed. (averted crash by returning 0 filled values)." );
        return &Terrible;
    }
    bool Inc()
    {
...
float BSPNode::intersects( const Vector &start, const Vector &end, Vector &norm ) const
{
    ...
    if (peq1 == 0 && peq2 == 0) {
        //on the plane; shouldn't collide unless the plane is a virtual plane
        float retval = 0;
        norm = n;
        if (front != NULL)
...
BSPTree::BSPTree( const char *filename )
{
    ...
    for (int a = 0, ip = 0; a < numRecords; a++) {
        nodes[a].x = VSSwapHostFloatToLittle( auxbuf[ip] );
        ip += sizeof (float);
        nodes[a].y = VSSwapHostFloatToLittle( auxbuf[ip] );
        ip += sizeof (float);
        nodes[a].z = VSSwapHostFloatToLittle( auxbuf[ip] );
        ip += sizeof (float);
        nodes[a].d = VSSwapHostFloatToLittle( auxbuf[ip] );
        ip += sizeof (float);
        nodes[a].isVirtual = false;
        nodes[a].hasFront = (auxbuf[ip] != 0);
        ip += sizeof (char);
        nodes[a].hasBack = (auxbuf[ip] != 0);
        ip += sizeof (char);
    }
    delete[] auxbuf;
