...
inline QVector SwizzleIt( const QVector &v )
{
    return QVector( -v.i, -v.j, v.k );
}                                                                           //return Vector (v.i,v.k,v.j);}
...
///make sure ~Planet destructors alloc memory for this one so it survives planet
    Matrix *xform;
public:
    QVector ReverseReverseX( const QVector &v ) const
    {
        return QVector( v.i, scaleheight*v.j, v.k );
    }
    QVector ReverseX( const QVector &v ) const
    {
        return QVector( v.i, v.j/scaleheight, v.k );
    }
    PlanetaryTransform( float radius, float xsize, float ysize, int ratio, float scaleatmos ) : SphericalTransform( xsize*ratio,
                                                                                                                    radius,
                                                                                                                    ysize*ratio )
    {
    ...
    {
        return ::Transform( *xform, SwizzleIt( SphericalTransform::Transform( ReverseReverseX( v ) ) ) );
    }
