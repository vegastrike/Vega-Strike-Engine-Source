...
float GetStarSpreadScale() {
    return configuration().graphics.star_spreading * configuration().physics.game_speed;
}

float ScaleOrbitDist(bool autogenerated) {
    if (autogenerated) {
        return configuration().physics.autogen_compactness * configuration().physics.runtime_compactness;
    }
    return configuration().physics.runtime_compactness;
}
...
void StarSystem::LoadXML(const string filename, const Vector &centroid, const float timeofyear) {
    ...
    xml->scale *= configuration().physics.star_system_scale;
    ...
    xml->reflectivity = configuration().graphics.reflectivity;
    xml->unitlevel = 0;
