...
#define DEFAULT_FACTION_SAVENAME "FactionTookOver_"
    ...
    void IOmessage(int delay, string from, string to, string message) {
        if (to == "news" && (!game_options()->news_from_cargo_list)) {
    ...
    void receivedCustom(int cp, bool trusted, string cmd, string args, string id) {
        ...
        securepythonstr(id);
        string pythonCode = vega_config::config->general.custom_python + "(" + (trusted ? "True" : "False")
                            + ", r\'" + cmd + "\', r\'" + args + "\', r\'" + id + "\')\n";
...
//    ::Python::reseterrors();
    ...
    string getSaveInfo(const std::string &filename, bool formatForTextbox) {
        ...
        savegame.ParseSaveGame(filename,
                               ...
                               true,
                               false,
                               vega_config::config->general.quick_savegame_summaries,
                               true,
                               true,
                               campaign_score_vars);
        ...
        if (!vega_config::config->general.quick_savegame_summaries) {
            bool hit = false;
