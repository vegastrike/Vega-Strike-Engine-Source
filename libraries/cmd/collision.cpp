...
Collision::Collision(Unit *unit, const QVector &location, const Vector &normal) :
    ...
    mass = std::max(unit->getMass(), vega_config::config->physics.minimum_mass);
...
void Collision::dealDamage(Collision other_collision, double deltaKE_linear, double deltaKE_angular) {
    ...
    Damage damage(0.5 * (deltaKE_linear + deltaKE_angular) /
            vega_config::config->physics.kilojoules_per_damage);
...
void Collision::collide(Unit *unit1,
    ...
    */
    ...
    double I1 =
            std::max(unit1->GetMoment(), vega_config::config->physics.minimum_mass) * unit1->radial_size * unit1->radial_size
                    * 0.667; // deriving scalar moment of inertia for unit 1
    double I2 =
            std::max(unit2->GetMoment(), vega_config::config->physics.minimum_mass) * unit2->radial_size * unit2->radial_size
    ...
    double mass2 = std::max(unit2->GetMass(), vega_config::config->physics.minimum_mass); // avoid subsequent divides by 0
