...
Collision::Collision(Unit *unit, const QVector &location, const Vector &normal) :
    ...
    mass = std::max(static_cast<double>(unit->getMass()), vega_config::config->physics.minimum_mass);
...
void Collision::dealDamage(Collision other_collision, double deltaKE_linear, double deltaKE_angular) {
    ...
    Damage damage(0.5 * (deltaKE_linear + deltaKE_angular) /
            vega_config::config->constants.kj_per_unit_damage);
...
void Collision::validateCollision(const QVector &relative_velocity,
    ...
    double RestorativeVelAlongNormal =
            -1 * (1 - vega_config::config->physics.inelastic_scale) * relative_velocity.Dot(normal1);
...
void Collision::collide(Unit *unit1,
    ...
    */
    ...
    double I1 =
            std::max(static_cast<double>(unit1->GetMoment()), vega_config::config->physics.minimum_mass) * unit1->radial_size * unit1->radial_size
                    * 0.667; // deriving scalar moment of inertia for unit 1
    double I2 =
            std::max(static_cast<double>(unit2->GetMoment()), vega_config::config->physics.minimum_mass) * unit2->radial_size * unit2->radial_size
    ...
    double mass2 = std::max(static_cast<double>(unit2->GetMass()), vega_config::config->physics.minimum_mass); // avoid subsequent divides by 0
    ...
    double impulse_magnitude = -1 * ((2 - vega_config::config->physics.inelastic_scale) * relative_velocity).Dot(normal1) /
