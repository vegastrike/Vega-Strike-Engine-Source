...
void Damageable::ApplyDamage(const Vector &pnt,
    ...
    const bool no_dock_damage = vega_config::config->physics.no_damage_to_docked_ships;
    const bool apply_difficulty_enemy_damage = vega_config::config->physics.difficulty_based_enemy_damage;
    ...
    if (shooter_is_player) {
        ...
        int anger = inflicted_armor_damage ? vega_config::config->ai.hull_damage_anger : vega_config::config->ai.shield_damage_anger;
...
//        const float hull_dam_to_eject = vega_config::config->eject_config.hull_damage_to_eject;
    ...
    if (previous_hull_percent >= vega_config::config->ai.hull_percent_for_comm &&
            unit->hull.Percent() < vega_config::config->ai.hull_percent_for_comm &&
        ...
        if (computer_ai && player) {
            ...
            if (computer_ai_state != nullptr && player_ai_state &&
                ...
                if (shooter_is_player && vega_config::config->ai.assist_friend_in_need) {
...
void Damageable::DamageRandomSystem(InflictedDamage inflicted_damage, bool player, Vector attack_vector) {
    ...
    if (!(hull_damage || (vega_config::config->physics.system_damage_on_armor && armor_damage))) {
        return;
    }
    ...
    // Therefore,
    double indiscriminate_system_destruction = vega_config::config->physics.indiscriminate_system_destruction;
...
void Damageable::DamageCargo(InflictedDamage inflicted_damage) {
    ...
    if (vega_config::config->physics.separate_system_flakiness_component) {
        return;
    }
...
bool Damageable::flickerDamage() {
    ...
    if (diff > vega_config::config->graphics.glow_flicker.flicker_time) {
        counter = getNewTime();
        diff = 0;
    }
    float tmpflicker = vega_config::config->graphics.glow_flicker.flicker_time * damagelevel;
    if (tmpflicker < vega_config::config->graphics.glow_flicker.min_flicker_cycle) {
        tmpflicker = vega_config::config->graphics.glow_flicker.min_flicker_cycle;
    }
    ...
    if (vega_config::config->graphics.glow_flicker.flicker_off_time > diff) {
        if (damagelevel > vega_config::config->graphics.glow_flicker.hull_for_total_dark) {
            return rand() > RAND_MAX * GetElapsedTime() * vega_config::config->graphics.glow_flicker.num_times_per_second_on;
        } else {
            return true;
        }
    }
