...
float accelStarHandler(float &input) {
    return input / (vega_config::config->physics.game_speed * vega_config::config->physics.game_accel);
}

float speedStarHandler(float &input) {
    return input / vega_config::config->physics.game_speed;
}
...
Movable::Movable() : cumulative_transformation_matrix(identity_matrix),
    ...
    const Vector default_angular_velocity(vega_config::config->general.pitch,
            vega_config::config->general.yaw,
            vega_config::config->general.roll);
    cutsqr = vega_config::config->warp.warp_stretch_cutoff * vega_config::config->warp.warp_stretch_cutoff;
    outcutsqr = vega_config::config->warp.warp_stretch_decel_cutoff * vega_config::config->warp.warp_stretch_decel_cutoff;
...
void Movable::UpdatePhysics(const Transformation &trans,
    ...
    if (resolveforces) {
        ...
        float velocity_max = vega_config::config->physics.velocity_max;
...
void Movable::AddVelocity(float difficulty) {
    ...
    float warprampuptime = playa ? vega_config::config->warp.warp_ramp_up_time : vega_config::config->warp.computer_warp_ramp_up_time;
    //Warp Turning on/off
    if (graphicOptions.WarpRamping) {
        ...
        if (unit->ftl_drive.Enabled()) {             //Warp Turning on
            ...
            graphicOptions.RampCounter = vega_config::config->warp.warp_ramp_down_time;
        }
        ...
        if (oldrampcounter != 0 && graphicOptions.RampCounter != 0) {
            if (unit->ftl_drive.Enabled()) {             //Warp is turning on before it turned off
                graphicOptions.RampCounter *= (1 - oldrampcounter / vega_config::config->warp.warp_ramp_down_time);
    ...
    if (unit->ftl_drive.Enabled() || graphicOptions.RampCounter != 0) {
        ...
        if (graphicOptions.RampCounter != 0) {
            ...
            if (!unit->ftl_drive.Enabled() && graphicOptions.RampCounter > vega_config::config->warp.warp_ramp_down_time) {
                graphicOptions.RampCounter = (1 - graphicOptions.RampCounter / warprampuptime) * vega_config::config->warp.warp_ramp_down_time;
            }
            ...
            rampmult = (unit->ftl_drive.Enabled()) ? 1.0F
                    ...
                    / vega_config::config->warp.warp_ramp_down_time) * (graphicOptions.RampCounter / vega_config::config->warp.warp_ramp_down_time);
        }
    ...
    graphicOptions.WarpFieldStrength =
            lastWarpField * vega_config::config->warp.warp_memory_effect + (1.0 - vega_config::config->warp.warp_memory_effect) * graphicOptions.WarpFieldStrength;
...
Vector Movable::ResolveForces(const Transformation &trans, const Matrix &transmat) {
    ...
    if (isPlayerShip()) {         //clamp to avoid vomit-comet effects
        caprate = vega_config::config->physics.max_player_rotation_rate;
    } else {
        caprate = vega_config::config->physics.max_non_player_rotation_rate;
    }
    ...
    if ((newbig && !oldbig) || (oldoutbig && !newoutbig)) {
        ...
        if (docache && !vega_config::config->graphics.in_system_jump_animation.empty()) {
            UniverseUtil::cacheAnimation(vega_config::config->graphics.in_system_jump_animation);
            docache = false;
        }
        ...
        float tmpsec = oldbig ? vega_config::config->warp.warp_stretch_decel_cutoff : vega_config::config->warp.warp_stretch_cutoff;
        UniverseUtil::playAnimationGrow(vega_config::config->graphics.in_system_jump_animation,
...
//    QVector qv = v.Cast();
    ...
    float minimum_multiplier = vega_config::config->warp.warp_multiplier_max * graphicOptions.MaxWarpMultiplier;
    Unit *nearest_unit = nullptr;
    minimum_multiplier = unit->CalculateNearestWarpUnit(minimum_multiplier, &nearest_unit, true);
    float minWarp = vega_config::config->warp.warp_multiplier_min * graphicOptions.MinWarpMultiplier;
    float maxWarp = vega_config::config->warp.warp_multiplier_max * graphicOptions.MaxWarpMultiplier;
...
//    const float warp_max_effective_velocity = vega_config::GetGameConfig().GetFloat("physics.warpMaxEfVel", default_max_warp_effective_velocity);
    const float warp_max_effective_velocity = vega_config::config->warp.max_effective_velocity;
