...
std::string GenerateAutoError(Unit *me, Unit *targ) {
    if (UnitUtil::isAsteroid(targ)) {
        return vega_config::config->graphics.hud.asteroids_near_message;
    }
    if (targ->isPlanet()) {
        return vega_config::config->graphics.hud.planet_near_message;
    }
    if (targ->getRelation(me) < 0) {
        return vega_config::config->graphics.hud.enemy_near_message;
    }
    return vega_config::config->graphics.hud.starship_near_message;
}
...
bool JumpCapable::AutoPilotToErrorMessage(const Unit *target,
    ...
    if (this != target) {
        if ((end - start).MagnitudeSquared()
            ...
            failuremessage = vega_config::config->graphics.hud.already_near_message;
            return false;
        }
